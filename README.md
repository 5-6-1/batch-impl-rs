...
**v0.9.4** (2026-08-25) — **macro-meta carrier rebuild + blanket-delegation and `#delegate`-rename enhancements**: `#blanket` delegates generic associated types (`type Iter<'a> = <T as Trait>::Iter<'a> where Self: 'a`), reports bare-`Self` parameters/returns with guidance (`Self::Assoc` returns pass), and the `@?` wrapper suffix adds `T: ?Sized` (`Box@?` → unsized targets like `Box<dyn Trait>`); `#delegate(size = len)` renames the delegated target method (the `#[call(...)]` mechanism of the `delegate` crate) — rename/`@all` overlap merges, double rename errors; the macro-meta layer was rebuilt internally — every reserved name replaced by structured carriers (fresh declarations/references as `@{g_i}`, variadic-segment template markers as the `[Prefix; ()]` array shape that cannot exist in compilable code, segment slots keyed by `(prefix, position)` pairs); generated impls show readable fresh generics (`P0, P1, ...`, collisions escaping as `P0A`, `P0B`, ... — the numbering never skips; the `@{A_pos}` body-slot spelling of this version was removed in the next development cycle — repeat blocks splice bound elements directly); `X<>` syncs inside `+`-joined bounds; fresh ranges re-open in impl bodies; repeat blocks gain round separators and fresh-driven cursor-only blocks (`impl{@0..}` + `@@N` name refs). `@all_fresh` deprecated (write `@0..`).
