...
batch_preview! {
    #[batch_impl(<T> Sortable<T> [Box, Rc]^Vec<T> where T: Ord { fn is_sorted(&self) -> bool; })]
    trait Sortable<T> { fn is_sorted(&self) -> bool; }
}
```

The preview reports one message containing all four generated impls. It also teaches the
