...
pub fn parse_single_prefix(tokens: &[TokenTree]) -> Result<PrefixItem, String> {
    ...
    match tokens {
        ...
        [TokenTree::Ident(id)] => Ok(PrefixItem::Container(id.clone())),
        // 检测 (T) 无逗号的情况，提示用户加逗号
...
pub fn apply_caret(prefix: &PrefixItem, target: &TargetItem) -> Result<TokenStream2, String> {
    match (prefix, target) {
        ...
        (PrefixItem::Container(name), TargetItem::Single(ts)) => Ok(quote! { #name < #ts > }),
        (PrefixItem::Container(name), TargetItem::Multi(args)) => {
            Ok(quote! { #name < #(#args),* > })
        }
