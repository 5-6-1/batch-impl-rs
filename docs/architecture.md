...
**v0.9.2** (unreleased) — `@N..` / `@N..M` fresh ranges fold into single-token placeholders (`_Param_{N}_With[_M]_BatchGen_`, `ast/fresh.rs`) at parse time and re-open against the impl's fresh list at codegen (`codegen/range_refs.rs::expand_range_refs`) — a range now works anywhere a single `@N` can (where predicates, `<>` generic args, tuple targets); variadic segments auto-complete a trailing comma in tuple templates (`preprocess/varseg.rs`); historical pre-0.9 changelog entries restored to the `^` operator of their time;
