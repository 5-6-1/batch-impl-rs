...
lib.rs              macro entry (#[batch_impl] / #[batch_impl_only] / batch_trait! / test macros) + module tree
  ...
  │   ├── fresh.rs          fresh/slot-carrier protocol (`FreshRef` + `SegRef`: two-way spell/parse, generate/construct/parse trio)
  ...
  │   ├── fresh.rs          fresh display naming (`FreshCtx`: doc-order P0.. numbering, collisions escape by spreadsheet-style letter suffixes `P0A`/`P0B`...) + `@N`/`@g_i` reference validation (target type / trait args)
