...
- **`_` wildcard in shape templates** (`codegen/shape.rs::match_ty`): `syn::Type::Infer` and array-length `syn::Expr::Infer` match any leaf position and stay `_` (never bound into the mapping) — `impl{B<_>}` / `impl{[A; _]}`. Tests in `tests/features/ext2_shape_forms.rs`
