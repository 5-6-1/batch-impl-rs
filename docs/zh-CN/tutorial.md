...
**v0.9.0**（unreleased）——apply 运算符重命名：`.` 是右结合 apply 运算符，**空格应用取代 `-`** 作为左结合组合（见 §2）；DSL 是**块的任意组合**（声明 / 指令块 / 代码块 / 类型任意顺序，`apply` 折叠）；同名泛型声明合并进 where；`_` 是 `impl{...}` 模板中永不替换的通配；`X<>` 经开关模板（`impl{@trait<>}` / `impl{Tr<>}`）同步为本 spec trait 应用；
