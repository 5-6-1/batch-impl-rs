...
**v0.9.2**（2026-08-21）——`@N..` / `@N..M` fresh 范围可以在**任何单个 `@N` 能出现的位置**使用：where 谓词（`@1..::Output: Clone`，尾部逐 fresh 复制）、`<>` 泛型实参、impl 泛型声明（`<@0..>`）、元组目标——另有**组内范围** `@L_N..` / `@L_N..M` 在单个生成器组内（跨数组分发稳定）。变长段不再需要尾随逗号（`impl{(A@..)}`）。0.9 之前 changelog 条目恢复使用当时的 `^` 运算符（0.9.0 发布时被改写成 `.`）。
