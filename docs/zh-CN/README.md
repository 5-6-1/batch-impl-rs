...
**v0.7.0**（2026-08-08）——0.6.7 已发布；0.7.0：**splat** `*` 前缀（摊平容器/生成器到列表，左操作数 `*[...]` 分配 / `*(...)` 追加）、数组分发传播（嵌套 `[A,B]` 笛卡尔积）、生成器 fresh 声明修复、`#fill` 单元素推荐（`#name{...}`）。开放扩展（`{! ...}`）、`@all_fresh` / `@N..M` 批量 where 引用、错误聚合。
...
| 特性                                 | 一句话                                      | 教程章节    |
|--------------------------------------|---------------------------------------------|-------------|
| 并列列表 `[A, B]`                    | 为多个类型同时实现，body 复用               |
| splat `*` 前缀                      | 摊平容器/生成器进外层列表——列表内拼接、`^` 右操作数扁平追加、泛型多实参；左操作数 `*[...]` 分配 / `*(...)` 追加 | 列表 | 列表与 body |
