...
**Unreleased**（开发中）——body 侧段引用现在**直接拼接**：`repeat_drivers.rs::substitute` 对照 `Mapping::seg_value` 解析 `@ident`、把绑定的叶子子树直接拼进每轮输出（`$( ... )*` 语义）；`SegRef`/`seg_ref_tokens` 载体从 `ast/fresh.rs` 退役、`apply_mapping` 只处理用户槽 ident（映射应用条件收窄到槽通道）、body 里非 fresh 的 `@{...}` 报错并给指导而非作为段载体透传。段旁的显式固定元素（`impl{(A0, @A..,)}`）走普通槽通道绑定；`@A..` 不衍生任何名字。
