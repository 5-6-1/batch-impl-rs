...
**v0.9.5**（2026-08-27）——**直接拼接收尾 + impl 模板家族**：body 侧段引用现在**直接拼接**——`repeat_drivers.rs::substitute` 对照 `Mapping::seg_value` 解析 `@ident`、把绑定的叶子子树直接拼进每轮输出（`$( ... )*` 语义）；`SegRef`/`seg_ref_tokens` 载体从 `ast/fresh.rs` 退役、`apply_mapping` 只处理用户槽 ident（映射应用条件收窄到槽通道）、body 里非 fresh 的 `@{...}` 报错并给指导而非作为段载体透传。段旁的显式固定元素（`impl{(A0, @A..,)}`）走普通槽通道绑定；`@A..` 不衍生任何名字。repeat 机制统一跑在一个 `RepeatCtx { segs, map, fresh, binding, budget }` 上（贯穿 `expand_repeat_blocks` / `expand_stream` / `expand_block` / `expand_nested` / `substitute`），`@{@N}`（载体内游标）解析逐轮 fresh 名。impl 模板家族：裸 `impl` 与裸 `where` 同款收集（`where_process.rs::kw_process` 共享收集器；`impl_process` 在 `mark_varseg` 之前）、相邻裸区拆分、`impl{...}` 附件按 depth-0 逗号并列切分（`extract.rs::split_impl_attachments`），每段独立分类为 fresh 绑定开关 / `@{N}` body 槽开关 / 形状模板；body 槽规则收紧（body 里的 `@{N}` 需要 `impl{@{}}` 或 fresh 绑定开关；宏注入的组/范围载体按形状豁免）。`angle_collect` 进入 `impl{...}` 组、`render_angles` 在 syn 解析前还原 flat `<>`。另有：`pipeline.rs` 拥有 codegen 阶段顺序权威；`util/subst.rs` 是唯一路径感知替换器（where 继承 + 指令 body）；`testing::GuardAlloc`（256 MiB）把测试中的超大分配变成 proptest 能捕获的 panic；MSRV 1.95（if-let guard、`Cell::update`）；proc-macro2 启用 `span-locations` 供开放 range 端点邻接判定。
...
**语法面冻结（0.7.2）**：全部既有记号的语义视为 final——后续版本只做加法
（新指令/新常量/新工具）、诊断精化与文档，对既有语义的任何改动都是刻意
的破坏性发布（`@N` 稳定性承诺自此推广到整个语法面）。
