...
- **fuzz 词表够不到标记 pass**——随机 ident 池里没有 `impl`，于是 `impl{...}` 模板（以及 `mark_varseg` → `mark_template`，残留检查唯一的热调用方）从未被生成：该 pass 零随机覆盖，而 fuzz 声称覆盖整条管线。已补入，256 用例语料现在真正跑到该守卫。
- **生产代码清空 panic 构造**（残留发现后的普查）——proc macro 里的 panic 就是编译器 ICE，故生产路径现在不含 `unwrap` / `expect` / `panic!` / `unreachable!` / `debug_assert!` / `assert!`（其余出现全在 `#[cfg(test)]` / 测试辅助文件）。具体：
  - `apply/apply_tuple.rs`——range 长度的 `debug_assert_eq!` 改为报内部错误诊断（长度按构造精确：空区间检查保证 `end >= start`，`check_expand_limit` 在 range 分配前就拒掉唯一会溢出的情形）；
  - `parse/ident_blocks.rs`——四处受守卫 `.unwrap()`（`+` bound 头、`::` 两个冒号、宏调用组）改为 `let else` 提取；
  - `ast/fresh.rs` / `parse/mod.rs`——两处 `_ => unreachable!("matched above")` 运算符宽度臂改为直接读守卫已确立的 `inclusive` 标志；
  - `util/scan.rs`——`Cursor::bump` 现在**夹取**、`advance` 饱和，确立文档化的位置不变量（`pos <= len`），`slice_since` / `take_segment` / `take_rest` 据此无需自身边界守卫（它们本是 `&tokens[pos..]`，一次越界 advance 就 panic）；回归测试 `cursor_position_never_exceeds_len`；
  - `entry/mod.rs`——`batch_trait!` trait 路径扫描的 `bump(); bump()` 改为 `advance(2)`（夹取）。其"游标落在 `::` 第二个冒号上"的前置条件（两次 bump 唯一可能越界的途径）不可达（`:` token 必走冒号臂，该臂消费整对）——但该不变量现在是结构性的，而非论证出来的。
...
- **canary 文档 + 共享形状判定定稿**（评审尾巴）——`stream.rs` 模块文档仍声称"每个自由函数带输入不变量 canary"（迁移后已过时）、`angle_collect` 注挂在已不存在的守卫体系下——两者重写为描述真实的 `mark_template` 后置条件。"cannot drift" 承诺现在兑现：标记循环改用 `is_unmarked_segment_at`（共享判定）而非内联副本，检测与消费字面共用同一函数（上一轮回归的根源正是两者不一致）。
- **`#delegate` 重命名缺左侧是用户错误，不是 panic**（评审 P1）——`#delegate(=foo)` 让 `eq - 1` 在 debug 构建下算术下溢 panic（release 回绕成 `usize::MAX` → 落到友好错误，debug/release 行为分叉）。改用 `eq.checked_sub(1)`——no-panic 承诺恢复。回归 fixture `tests/ui/delegate_rename_missing_left.rs`。
- **排他 `@N..M` 在类型位置统一为含端点协议**（评审 P2，冻结面不一致）——`@0..2` 在 `<>` 实参 / 元组目标 / impl 泛型 bound 展开成 `P0, P1, P2`（parse 层范围折叠保留原始端点），而 where 谓词路径归一化为 `..=M-1`（`P0, P1`）——同一拼写两种语义。协议文档写 "`@N..M` / `@N..=M` normalized to inclusive"；parse 层现在归一化排他形式（`..M` → `Closed(M-1)`），与 where 谓词解析一致（`at_ref_block`、`resolve_at_refs`、`parse_fresh_switch`——后者的注释声称支持 `@N..M` 而代码返回 `None`）。空排他范围（`@2..1`）报定向错误。功能测试 `exclusive_range_in_type_position_is_exclusive`。
- **bare-`Self` blanket 检测递归进组**（评审 P3）——`ty_refs_bare_self` 只扫顶层 token，`(Self, u8)` / `Box<Self>` 内的 `Self` 漏检、产出以晦涩 rustc E0308 呈现的错误委托。现在递归进组。fixture `tests/ui/blanket_self_in_group.rs`。
...
- **裸 `impl` 的 impl-Trait 目标报定向诊断**（评审 P1）——解析层一直容忍 `impl Fn() -> u8` / `impl dyn Fn() -> u8` / `impl Iterator + Clone` 作为目标拼写（`parse/mod.rs::impl_trait_parses` 锁定；渲染输出从来不是合法 Rust——双重 `impl`——已在 0.9.4 上实测验证），但 0.9.5 的裸 impl 收集把它静默改写成形状模板，渲染出**空**目标类型（`impl T1 for {}`）。`impl_process` 现在把收集到的区域（fn 族 / `dyn` / `for` 开头，或深度 0 `+` 链）分类为 impl-Trait 目标并报定向诊断，指引 `dyn Fn() -> u8` / `impl{...}`；解析层容忍与其单元测试不变。UI fixture `tests/ui/bare_impl_trait_target.rs`。
- **impl 条目剥除槽位同名的 item 泛型**（评审 P2）——`assemble_impl` 原样保留 item 的 `<>` 声明、槽映射却重写了 for-Type/body，于是 `impl<T> Mk for Wrapper<T>` 配模板槽位 `T` 产出 `impl<T> Mk for Box<u8>` → rustc E0207 且无宏级提示。名字是映射槽位的参数现在被剥除（它是替换目标，不是声明），其 bound 转为替换后类型上的 where 谓词（`impl<T: Clone>` → `where u8: Clone`）而非静默丢弃。测试 `impl_entry_slot_named_item_generic_stripped` / `impl_entry_slot_named_param_bounds_become_where`。
- **裸 `where` 逗号边界**（评审 P3）——`scan_body_boundary` 在后续块不是谓词时，让无 body 的 where 区域在深度 0 `,` 处结束（`usize where T: Clone, isize`——`isize` 不可能是谓词——把 `, isize` 留作下一个 `,` 分隔的 spec；`where A: Clone, B: Copy` 继续扫描）。前瞻（`chunk_is_predicate`）复用区域的停止条件；angle 组已是不透明组，扁平扫描不会看到 `<>` 内部。测试 `bare_where_comma_splits_specs` / `bare_where_comma_keeps_predicates` + 特性测试 `comma_after_bare_where_splits_specs`。
- **`fold_flat_refs` 报畸形引用**（评审 P3）——where 谓词路径把空排他范围（`@2..2`）等畸形 `@` 拼写原样留下，`@` 泄漏进渲染出的子句（`where @ 2 .. 2 : Clone`），而类型位置路径会报错。折叠现在返回 `Result`，与类型路径报同款诊断（非位置 token、非数字范围端点、空排他范围）。UI fixture `tests/ui/where_empty_exclusive_range.rs`。
