...
- **左操作数按来源分语义**：`TySplat` 是镜像 parse 定界符的枚举——`TySplat::Array`
  分配 `^T`（`*[A^T,B^T]`——集合，对标 `TyArray`）、`TySplat::Tuple` 追加
  （`*(A,B,...,T)`——列表，对标 `TyTuple`）；`*()^N`（空 splat）保持 splat 形态——`T^*()^2` =
  `<A,B>T<A,B>`（经载体生成位置 fresh 对；裸 `*()^N` 单独作目标会被 rustc 拒绝——多个 impl 共享
  一份泛型声明但各自只用其中一个参数，E0207）；**非空 splat 的幂（`*(A,B)^N`）以专用诊断拒绝**——
  splat 摊平会重复笛卡尔元组组合（E0119）；要笛卡尔积直接用 `(A,B)^N`，要 fresh 泛型用
  `T^*()^N`；裸 `*()` 单独作目标产生**零 impl**（空列表无元素）；左操作数
