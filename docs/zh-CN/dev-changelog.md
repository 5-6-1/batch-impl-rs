...
- **左操作数按来源分语义**：`TySplat` 是镜像 parse 定界符的枚举——`TySplat::Array`
  ...
  （`*(A,B,...,T)`——列表，对标 `TyTuple`，包回）；**splat 上的 `^N` 幂产生离开 splat 的最终形态**：`*(A,B)^2` =
  `*[(A,A),(A,B),(B,A),(B,B)]`（笛卡尔组合原样分发——包回会把元组组合再摊平成重复，E0119）；`*()^N`（空
  splat）把 fresh 元组包回 splat 以便载体追加参数（`T^*()^2` = `<A,B>T<A,B>`；裸 `*()^N` 单独作目标会被
