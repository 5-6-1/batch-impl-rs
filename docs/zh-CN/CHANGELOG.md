...
- **变长段与重复块（Ext 2）**——`impl{...}` 模板可用 `ident@..` 声明**变长段**：覆盖从自身位置起的所有剩余元组位置，名字**对齐叶子位置**（`(u8, A@..,)` 匹配 `(u8, u16, u32)` → `A1`、`A2`；`(A@..,)` 任意 arity → `A0..An`），同层多段均分剩余位置（无法均分 / 段名前缀重复报错），段可递归进嵌套元组。body 用 `@(...)..` 重复：`@ident` 为第 i 个元素的槽名（随后由槽映射改写成绑定值），`@N` 为索引游标（展开为 `N + i`，路径前缀由用户书写），块按驱动段的公共长度逐轮输出（引用的段必须等长；块的长度有三个来源：块内 `@ident` 引用、**前置段声明** `@A(...)..`（`@` 后直接写段名，支持纯游标块）、或纯游标块用模板**唯一段**），嵌套块独立轮次（笛卡尔积），块体尾部 `,` 为每轮分隔符（并列块之间不写逗号）。一条 spec 覆盖形状的所有元组 arity——alga2 风格 `()^1..=4 where{@all_fresh: Magma} impl{(A@..,)} #combine{( @(@A::combine(&self.@0, &rhs.@0),).. )}` 为 n = 1..4 生成 `impl<A0..An> Magma for (A0, ..., An) where A0: Magma, ...`
