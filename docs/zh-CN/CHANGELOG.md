...
- 嵌套幂等、空 splat 无操作；**左操作数按来源括号分语义**：`*[...]^T` 分配（`*[A^T,B^T]`——集合，对标 `TyArray`）、`*(...)^T` 追加（`*(A,B,...,T)`——列表，对标 `TyTuple`）；**`*(A,B)^N` 幂分发笛卡尔组合**——`*(A,B)^2` = `*[(A,A),(A,B),(B,A),(B,B)]`（4 个元组 impl）；`*()^N` 保持 splat 形态供载体——`T^*()^2` = `<A,B>T<A,B>`；`#fill` 单元素推荐——写 `#name{body}` 而非 `#fill(name){body}`；`*const`/`*mut` 指针不受影响
